...
import { SHIP_HULLS, type ShipHull } from "./ship-hulls.js";
...
export const ENEMY_MODEL_KEY: Record<string, string> = {
  scout: "enemy_scout",
  ...
  erix: "enemy_erix",
  angin: "enemy_angin",
  crobium: "enemy_crobium",
  draug: "enemy_draug",
  knoton: "enemy_knoton",
  maron: "enemy_maron",
  nabas: "enemy_nabas",
  silikum: "enemy_silikum",
  simonit: "enemy_simonit",
};
...
export const PLAYER_SIZE_SCALE: Record<string, number> = {
...
export function enemyModelKey(type: string, id: string): string | undefined {
  if (id.startsWith("pboss")) return "marauder";
  if (id.startsWith("pir")) {
    let h = 0;
    for (let ci = 4; ci < id.length; ci++) h = (h + id.charCodeAt(ci)) | 0;
...
export function enemySizeScale(size: number): number {
  return size / 15.2;
}

function pointInHull(pts: [number, number][], x: number, z: number): boolean {
  // even-odd ray cast
  let inside = false;
...
export function shipHitTest(
  hullKey: string,
  ex: number, ey: number, angle: number, sizeScale: number,
  px: number, py: number,
  inflate = 1,
): boolean {
  const hull: ShipHull | undefined = SHIP_HULLS[hullKey];
  ...
  if (dx * dx + dy * dy > rw * rw) return false;
 ...
 */
export function shipHitTestSwept(
  hullKey: string,
  ex: number, ey: number, angle: number, sizeScale: number,
  ...
  if (cx * cx + cy * cy > rw * rw) return false;
...
export function shipHullRadius(hullKey: string, sizeScale: number): number {
