...
export function rollLegendaryId(rnd, slot, ilvl) {
...
export function profileForEnemy(type, isBoss, isPirate, zoneTier) {
    if (isBoss)
        return LOOT_PROFILES.boss;
    if (isPirate)
        return LOOT_PROFILES.pirate;
    if (zoneTier >= 6)
        return LOOT_PROFILES.worldElite;
    if (type === "titan" || type === "juggernaut" || type === "dread" || type === "overlord" || type === "leviathan")
        return LOOT_PROFILES.heavy;
    if (type === "destroyer" || type === "sentinel" || type === "wraith" || type === "specter" || type === "phantom")
        return LOOT_PROFILES.veteran;
    if (type === "raider" || type === "corvette" || type === "voidling")
        return LOOT_PROFILES.standard;
...
export function rollRarity(rnd, ilvl, profile, luck) {
    const entries = [];
    for (const r of RARITY_ORDER) {
        ...
        if (r !== "common" && r !== "uncommon")
            w *= profile.rarityBonus * (1 + luck);
        entries.push([r, w]);
    }
    let rarity = pickWeighted(rnd, entries);
    const floor = profile.guaranteedMinRarity;
    if (floor && RARITY_ORDER.indexOf(rarity) < RARITY_ORDER.indexOf(floor))
        rarity = floor;
    return rarity;
}
export function rollAffixes(rnd, rarity, ilvl, slot) {
    ...
    for (let n = 0; n < count; n++) {
        ...
        for (const [id, a] of Object.entries(AFFIXES)) {
            if (!a.slots.includes(slot))
                continue;
            if (usedGroups.has(a.group))
                continue;
            if (a.minIlvl && ilvl < a.minIlvl)
                continue;
...
export function rollDrop(ctx) {
    ...
    const slot = pickWeighted(rnd, [["weapon", 40], ["generator", 30], ["module", 30]]);
    ...
    if (bases.length === 0)
        bases = BASE_POOL.filter((b) => b.slot === slot);
    const base = bases[Math.floor(rnd() * bases.length)];
    const affixes = rollAffixes(rnd, rarity, ilvl, slot);
    const legendaryId = rarity === "legendary" || rarity === "relic" || rarity === "celestial"
...
export function resolveAffixStats(item) {
    const out = {};
    if (item.affixes) {
...
// ── Display helpers ────────────────────────────────────────────────────────
export function rarityColor(r, fallback) {
    return r ? (RARITIES[r]?.color ?? fallback) : fallback;
}
export function affixLine(roll) {
    const a = AFFIXES[roll.id];
    if (!a)
        return "";
...
export function itemDisplayName(item, baseName) {
    const leg = item.legendaryId ? LEGENDARIES[item.legendaryId] : undefined;
    if (leg)
        return leg.name;
    ...
    let name = baseName;
    if (prefix)
        name = `${prefix.namePart} ${name}`;
    if (suffix)
        name = `${name} ${suffixWord(suffix.namePart)}`;
    return name;
}
function suffixWord(part) {
    return `of the ${part}`;
}
export function sellValue(item, basePrice) {
...
export function validateItem(item) {
    if (!item || typeof item.instanceId !== "string" || typeof item.defId !== "string")
        return "malformed";
    if (item.ilvl !== undefined && (item.ilvl < 1 || item.ilvl > ILVL_CAP))
        return "ilvl out of range";
    if (item.rarity !== undefined && !RARITIES[item.rarity])
        return "unknown rarity";
    if (item.legendaryId !== undefined) {
        const leg = LEGENDARIES[item.legendaryId];
        if (!leg)
            return "unknown legendary " + item.legendaryId;
        if (!item.rarity || RARITY_ORDER.indexOf(item.rarity) < RARITY_ORDER.indexOf("legendary"))
            return "legendary effect on low rarity";
        if ((item.ilvl ?? 1) < leg.minIlvl)
            return "legendary below min ilvl";
    }
    if (item.affixes) {
        if (item.rarity === undefined)
            return "affixes without rarity";
        const rdef = RARITIES[item.rarity];
        if (item.affixes.length > rdef.maxAffixes)
            return "too many affixes for rarity";
        const groups = new Set();
        const tierCap = affixTierForIlvl(item.ilvl ?? 1) + 1;
        for (const roll of item.affixes) {
            const a = AFFIXES[roll.id];
            if (!a)
                return "unknown affix " + roll.id;
            if (groups.has(a.group))
                return "duplicate affix group " + a.group;
            groups.add(a.group);
            if (roll.tier > tierCap)
                return "affix tier above ilvl bracket";
            const [lo, hi] = a.tiers[roll.tier - 1] ?? [0, 0];
            // allow rounding wiggle
            if (roll.value < lo - Math.abs(lo) * 0.01 - 0.51 || roll.value > hi * 1.01 + 0.51)
                return "affix value out of tier range";
        }
    }
    return null;
}
