// I-16-create-a-clan · CREATE A CLAN
//
// Das Formular. Links Clanname (3–24 Zeichen, prüft gegen bestehende Charters),
// Tag (3–4 Zeichen, wird großgeschrieben), Motto mit Restzähler, Wappenbau in vier
// Auswahlreihen — Form, Symbol, Wappenfarbe, Symbolfarbe — sowie Aufnahmemodus,
// Mindestlevel und Rekrutierungs-Tags (höchstens drei).
// 
// Rechts läuft alles live mit: die Registry-Vorschau zeigt das Wappen so, wie der
// Eintrag später im Directory steht. Darunter Ledger und vier Charter-Checks, die
// einzeln von rot auf grün springen — FILE CHARTER bleibt gesperrt, bis alle stehen.
//
...
//
//   import { mount } from "./I-16-create-a-clan";
...
const TAKEN_NAMES = ["Iron Covenant", "Ember Wake", "Silent Meridian", "Nova Praetorium",
  "Drift Collective", "Halcyon Vanguard", "Scrapline Union", "Ashen Compact", "Void Syndicate"];
const TAKEN_TAGS = ["IRC", "EMW", "SLM", "NVP", "DFC", "HLV", "SCU", "ASH", "VSD"];

const SHAPES: [string, string][] = [
  ["hex", "HEXAGON"], ["shield", "SHIELD"], ["diamond", "DIAMOND"],
  ["oct", "OCTAGON"], ["chev", "CHEVRON"], ["blade", "BLADE"],
];
const SYMBOLS: [string, string][] = [
  ["◆", "DIAMOND"], ["✦", "STAR"], ["▲", "SPEAR"], ["◉", "ORB"], ["❖", "PRISM"],
  ["✧", "SPARK"], ["⬟", "CORE"], ["⚔", "BLADES"], ["☓", "CROSS"], ["◈", "GATE"],
  ["▣", "BASTION"], ["✹", "NOVA"],
];
const CREST_COLORS = ["#b866ff", "#4ee2ff", "#5cff8a", "#e8b94d", "#ff4d5e", "#ff5cf0", "#ff8c4d", "#9fb6d4"];
const GLYPH_COLORS = ["#ffffff", "#0a0810", "#f5dda6", "#9df2ff", "#ffc9f6", "#c8ffd8"];
const FOCUS = ["SECTOR WAR", "BOUNTY", "MINING", "SALVAGE", "EXPLORATION", "DUNGEONS", "TRADE", "ESCORT"];
const FEE = 250000;

/** Wappenform als Punktliste. */
function crestShape(kind: string, x: number, y: number, w: number, h: number): number[] {
  ...
  switch (kind) {
    ...
    case "oct": {
      const q = w * 0.29;
      return [x + q, y, r - q, y, r, y + q, r, b - q, r - q, b, x + q, b, x, b - q, x, y + q];
    }
    ...
    default: return hexa(x, y, w, h);
  }
}

export type CreateClanOpts = WindowOpts & {
  credits?: number;
  onCreate?: (charter: {
    name: string; tag: string; motto: string; shape: string; symbol: string;
    crestColor: string; glyphColor: string; admission: string; minLevel: number; focus: string[];
  }) => void;
};

export function mount(o: CreateClanOpts = {}): WindowHandle {
  const credits = o.credits ?? 812000;
  const W = 1040, H = 604;
  const shell = windowShell({
    w: W, h: H, accent: ACCENT.action,
    title: "Create a Clan", note: "charter form · live crest preview",
    onClosed: o.onClosed, autoplay: o.autoplay,
  });

  let name = "", tag = "", motto = "";
  let shape = 0, symbol = 0, crestC = 0, glyphC = 0;
  let admission: "open" | "apply" | "invite" = "apply";
  let minLevel = 20;
  const picks: string[] = [];
  ...
  const fName = textInput({ w: 250, placeholder: "Name your charter", maxLength: 24, onInput: (v) => { name = v; toast = null; build(); } });
  const fTag = textInput({ w: 108, placeholder: "ABC", maxLength: 4, upper: true, onInput: (v) => { tag = v; toast = null; build(); } });
  const fMotto = textInput({ w: 372, placeholder: "One line your recruits will read first", maxLength: 64, onInput: (v) => { motto = v; build(); } });

  /** Auswahlreihe mit ‹ Wert › — nebeneinander, damit das Panel flach bleibt. */
  const picker = (into: PIXI.Container, x: number, y: number, w: number,
    label: string, value: string, accent: number, onStep: (d: number) => void): void => {
    const l = lbl(label, 6, 0xcedef6, 2.2);
    l.x = x; l.y = y;
    into.addChild(l);
    const g = new PIXI.Graphics();
    cut(g, x, y + 12, w, 26, 7, shade(accent, -0.42), 1, "tl-br");
    cut(g, x + 1.5, y + 13.5, w - 3, 23, 6, 0x0b0912, 1, "tl-br");
    g.rect(x + 6, y + 13.5, w - 12, 1).fill({ color: shade(accent, 0.7), alpha: 0.35 });
    g.rect(x + 4, y + 36, w - 8, 2).fill({ color: accent, alpha: 0.5 });
    into.addChild(g);
    const v = val(value, 9.5, 0xf2f7ff);
    v.anchor.set(0.5, 0); v.x = x + w / 2; v.y = y + 19;
    into.addChild(v);
    ([[-1, "‹", x + 2], [1, "›", x + w - 22]] as [number, string, number][]).forEach(([d, glyph, ax]) => {
      const a = new PIXI.Container();
      const ag = new PIXI.Graphics();
      ag.rect(0, 0, 20, 22).fill(shade(accent, -0.3));
      ag.rect(1, 1, 18, 20).fill(0x120e1c);
      ag.rect(2, 1, 16, 1).fill({ color: shade(accent, 0.7), alpha: 0.4 });
      a.addChild(ag);
      const gt = lbl(glyph, 10, shade(accent, 0.6), 0);
      gt.anchor.set(0.5); gt.x = 10; gt.y = 11;
      a.addChild(gt);
      a.x = ax; a.y = y + 14;
      states(a, { lift: 1, sink: 1, onClick: () => onStep(d) });
      into.addChild(a);
    });
  };
  ...
  const build = (): void => {
    ...
    const sw = 316, lw = shell.bodyW - sw - 14;
    sideL.x = lw + 14;
    const c = hex(CREST_COLORS[crestC]);
    const gc = hex(GLYPH_COLORS[glyphC]);

    const clean = name.trim();
    const nameTaken = TAKEN_NAMES.some((n) => n.toLowerCase() === clean.toLowerCase());
    const tagTaken = TAKEN_TAGS.includes(tag.toUpperCase());
    const nameOk = clean.length >= 3 && clean.length <= 24 && !nameTaken;
    const tagOk = /^[A-Z0-9]{3,4}$/.test(tag.toUpperCase()) && !tagTaken;
    const tagsOk = picks.length >= 1 && picks.length <= 3;
    const fundsOk = credits >= FEE;
    const ok = nameOk && tagOk && tagsOk && fundsOk;
    const done = [nameOk, tagOk, tagsOk, fundsOk].filter(Boolean).length;

    /* Name + Tag */
    const nl = lbl("CLAN NAME", 6, 0xcedef6, 2.2);
    leftL.addChild(nl);
    const nn = val(!clean ? "3–24 CHARACTERS" : nameTaken ? "ALREADY REGISTERED" : nameOk ? "AVAILABLE" : "TOO SHORT",
      8, !clean ? 0x6b7f96 : nameOk ? 0x5cff8a : 0xff8c9b);
    nn.anchor.x = 1; nn.x = 250; nn.y = 0;
    leftL.addChild(nn);
    fName.root.y = 12;
    leftL.addChild(fName.root);

    const tl = lbl("TAG", 6, 0xcedef6, 2.2);
    tl.x = 264;
    leftL.addChild(tl);
    const tn = val(!tag ? "3–4" : tagTaken ? "TAKEN" : tagOk ? "FREE" : "3–4",
      8, !tag ? 0x6b7f96 : tagOk ? 0x5cff8a : 0xff8c9b);
    tn.anchor.x = 1; tn.x = 372; tn.y = 0;
    leftL.addChild(tn);
    fTag.root.x = 264; fTag.root.y = 12;
    leftL.addChild(fTag.root);

    /* Motto */
    const ml = lbl("MOTTO", 6, 0xcedef6, 2.2);
    ml.y = 50;
    leftL.addChild(ml);
    const mn = val((64 - motto.length) + " LEFT", 8, 0x6b7f96);
    mn.anchor.x = 1; mn.x = 372; mn.y = 50;
    leftL.addChild(mn);
    fMotto.root.y = 62;
    leftL.addChild(fMotto.root);

    /* Wappenbau: vier Auswahlreihen nebeneinander */
    const pw = (lw - 18) / 2;
    picker(leftL, 0, 102, pw, "CREST SHAPE", SHAPES[shape][1], c, (d) => { shape = (shape + d + SHAPES.length) % SHAPES.length; build(); });
    picker(leftL, pw + 18, 102, pw, "INNER SYMBOL", SYMBOLS[symbol][1], c, (d) => { symbol = (symbol + d + SYMBOLS.length) % SYMBOLS.length; build(); });
    picker(leftL, 0, 152, pw, "CREST COLOUR", CREST_COLORS[crestC].toUpperCase(), c, (d) => { crestC = (crestC + d + CREST_COLORS.length) % CREST_COLORS.length; build(); });
    picker(leftL, pw + 18, 152, pw, "SYMBOL COLOUR", GLYPH_COLORS[glyphC].toUpperCase(), c, (d) => { glyphC = (glyphC + d + GLYPH_COLORS.length) % GLYPH_COLORS.length; build(); });

    /* Aufnahme + Mindestlevel */
    const al = lbl("ADMISSION", 6, 0xcedef6, 2.2);
    al.y = 206;
    leftL.addChild(al);
    ([["open", "OPEN"], ["apply", "APPLY"], ["invite", "INVITE"]] as const).forEach(([k, label], i) => {
      const on = admission === k, bw2 = (pw - 12) / 3;
      const b = makeTab({
        w: bw2, h: 24, label, accent: c, active: on,
        onClick: () => { admission = k; build(); },
      });
      b.x = i * (bw2 + 6); b.y = 218;
      leftL.addChild(b);
    });
    const lvl = lbl("MIN PILOT LEVEL", 6, 0xcedef6, 2.2);
    lvl.x = pw + 18; lvl.y = 206;
    leftL.addChild(lvl);
    ([[-5, "−", 0xff4d5e], [5, "+", 0x5cff8a]] as [number, string, number][]).forEach(([d, glyph, ac], i) => {
      const size = 24, b = new PIXI.Container();
      const g = new PIXI.Graphics();
      g.rect(0, 3, size, size).fill({ color: 0x03050a, alpha: 0.9 });
      g.rect(0, 0, size, size).fill(shade(ac, 0.45));
      g.rect(1.5, 1.5, size - 3, size - 3).fill(shade(ac, -0.36));
      g.rect(3, 3, size - 6, size - 6).fill(shade(ac, -0.74));
      g.rect(4, 3, size - 8, 1).fill({ color: shade(ac, 0.8), alpha: 0.7 });
      g.rect(3, size - 5, size - 6, 2).fill({ color: ac, alpha: 0.75 });
      b.addChild(g);
      const gt = new PIXI.Text({
        text: glyph,
        style: { fontFamily: "Orbitron, sans-serif", fontSize: 13, fontWeight: "700", fill: shade(ac, 0.8) },
      });
      gt.anchor.set(0.5); gt.x = size / 2; gt.y = size / 2;
      b.addChild(gt);
      b.x = pw + 18 + i * 78; b.y = 218;
      states(b, { onClick: () => { minLevel = Math.max(0, Math.min(60, minLevel + d)); build(); } });
      leftL.addChild(b);
    });
    const lv2 = val(String(minLevel), 15, 0xf2f7ff);
    lv2.anchor.set(0.5, 0); lv2.x = pw + 18 + 51; lv2.y = 222;
    leftL.addChild(lv2);

    /* Rekrutierungs-Tags */
    const rl = lbl("RECRUITING FOR", 6, 0xcedef6, 2.2);
    rl.y = 258;
    leftL.addChild(rl);
    const rn = val(picks.length + " / 3 PICKED", 8, tagsOk ? 0x5cff8a : 0x6b7f96);
    rn.anchor.x = 1; rn.x = lw; rn.y = 258;
    leftL.addChild(rn);
    let cx = 0, cy = 272;
    FOCUS.forEach((f) => {
      const on = picks.includes(f);
      const l = lbl(f, 6.5, on ? 0xf4ecff : 0x8fa4bc, 1.8);
      const w2 = l.width + 20;
      if (cx + w2 > lw) { cx = 0; cy += 26; }
      const b = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 0, 0, w2, 22, 5, on ? shade(c, -0.2) : 0x141020, 1, "tl-br");
      cut(g, 1, 1, w2 - 2, 20, 4.5, on ? shade(c, -0.62) : 0x0a0812, 1, "tl-br");
      g.rect(0, 0, 2, 22).fill({ color: c, alpha: on ? 1 : 0.35 });
      g.rect(4, 1, w2 - 8, 1).fill({ color: shade(c, 0.7), alpha: on ? 0.5 : 0.14 });
      b.addChild(g);
      l.x = 10; l.y = 8;
      b.addChild(l);
      b.x = cx; b.y = cy;
      states(b, {
        lift: 2, sink: 1,
        onClick: () => {
          if (on) picks.splice(picks.indexOf(f), 1);
          else if (picks.length >= 3) { toast = { text: "Three recruiting tags is the limit", hex: 0xff8c9b }; }
          else picks.push(f);
          build();
        },
      });
    ...
    sideL.addChild(card);
    const wash = new PIXI.Sprite(radTex([[0, rgba(c, 0.16)], [0.74, "rgba(0,0,0,0)"]]));
    wash.width = sw; wash.height = 160;
    sideL.addChild(wash);
    const pl = lbl("REGISTRY PREVIEW", 6.5, 0xcedef6, 2.6);
    pl.x = 16; pl.y = 14;
    sideL.addChild(pl);
    const step = val(done + " / 4 CLEARED", 9, ok ? 0x5cff8a : 0xc9a8ff);
    step.anchor.x = 1; step.x = sw - 16; step.y = 13;
    sideL.addChild(step);

    // Wappen in gewählter Form
    const cs = 76;
    crestSize = cs;
    const crest = new PIXI.Container();
    const cg = new PIXI.Graphics();
    for (const [inset, tone] of [[0, 0.5], [3, -0.24], [6, -0.62]] as [number, number][]) {
      cg.poly(crestShape(SHAPES[shape][0], inset, inset, cs - inset * 2, cs - inset * 2)).fill(shade(c, tone));
    }
    crest.addChild(cg);
    const cclip = new PIXI.Graphics();
    cclip.poly(crestShape(SHAPES[shape][0], 6, 6, cs - 12, cs - 12)).fill(0xffffff);
    ...
    cwash.width = cwash.height = cs;
    chost.addChild(cwash);
    ...
    const sym = new PIXI.Text({
      text: SYMBOLS[symbol][0],
      style: { fontFamily: "Orbitron, sans-serif", fontSize: cs * 0.42, fill: gc },
    });
    sym.anchor.set(0.5); sym.x = cs / 2; sym.y = cs * 0.47;
    crest.addChild(sym);
    const cgl = glow(c, cs * 1.8, cs * 1.8, 0.4);
    cgl.x = cgl.y = -cs * 0.4;
    crest.addChildAt(cgl, 0);
    glows.push({ sp: cgl, base: 0.34 });
    crest.x = 16; crest.y = 32;
    sideL.addChild(crest);

    const nv = lbl(clean ? clean.toUpperCase() : "UNNAMED CHARTER", 12, 0xeddcff, 1.2);
    nv.x = 104; nv.y = 38;
    sideL.addChild(nv);
    const tv = val(tag ? "[" + tag + "]" : "[···]", 10, 0xd6c8f2);
    tv.x = 104; tv.y = 56;
    sideL.addChild(tv);
    const lvv = lbl("LEVEL 1  ·  1 / 11 MEMBERS", 6.5, c, 2);
    lvv.x = 104; lvv.y = 74;
    sideL.addChild(lvv);
    const mv = txt(motto || "No motto filed yet.", 10, 0xcedef2, sw - 120);
    mv.x = 104; mv.y = 88;
    sideL.addChild(mv);

    let y = 122;
    const AD: Record<string, [string, number]> = {
      open: ["OPEN TO ALL", 0x5cff8a], apply: ["BY APPLICATION", 0x4ee2ff], invite: ["INVITE ONLY", 0xff8c4d],
    };
    let px2 = 16;
    const chips: [string, number][] = [[AD[admission][0] + (minLevel > 0 ? " · LV " + minLevel + "+" : ""), AD[admission][1]],
      ...picks.map((p) => [p, c] as [string, number])];
    for (const [text, hx] of chips) {
      const l = lbl(text, 6.5, shade(hx, 0.5), 1.8);
      const w2 = l.width + 16;
      if (px2 + w2 > sw - 16) { px2 = 16; y += 22; }
      const g = new PIXI.Graphics();
      cut(g, px2, y, w2, 18, 4, shade(hx, -0.62), 1, "tl-br");
      g.rect(px2, y, 2, 18).fill({ color: hx, alpha: 0.85 });
      sideL.addChild(g);
      l.x = px2 + 9; l.y = y + 6;
      sideL.addChild(l);
      px2 += w2 + 5;
    ...
    // Ledger
    const ll = lbl("FILING LEDGER", 6.5, shade(ACCENT.currency, 0.4), 2.6);
    ll.x = 16; ll.y = y;
    sideL.addChild(ll);
    y += 16;
    ([["FILING FEE", num(FEE), 0xe8b94d, 0xf5dda6],
      ["YOUR CREDITS", num(credits), fundsOk ? 0x5cff8a : 0xff4d5e, fundsOk ? 0xdbe9fb : 0xff8c9b],
      ["AFTER FILING", num(Math.max(0, credits - FEE)), 0x4ee2ff, 0xdbe9fb],
      ["STARTING SLOTS", "11", 0xb866ff, 0xdbe9fb]] as [string, string, number, number][])
      .forEach(([k, v, hx, vc]) => {
        const g = new PIXI.Graphics();
        g.rect(16, y, sw - 32, 22).fill(0x0a0810);
        g.rect(16, y, sw - 32, 1).fill({ color: 0x000000, alpha: 0.8 });
        g.rect(16, y + 21, sw - 32, 1).fill({ color: hx, alpha: 0.14 });
        g.poly([24, y + 8.5, 27.5, y + 5, 31, y + 8.5, 27.5, y + 12]).fill(hx);
        sideL.addChild(g);
        const kk = val(k, 8.5, 0xc4daee); kk.x = 38; kk.y = y + 6;
        const vv = val(v, 9, vc); vv.anchor.x = 1; vv.x = sw - 24; vv.y = y + 6;
    ...
    // Checks
    y += 8;
    const cl = lbl("CHARTER CHECKS", 6.5, 0xcedef6, 2.6);
    cl.x = 16; cl.y = y;
    sideL.addChild(cl);
    y += 16;
    ([[nameOk, nameTaken ? "Clan name is already registered" : "Clan name, 3–24 characters"],
      [tagOk, tagTaken ? "That tag belongs to another charter" : "Tag, 3–4 letters or digits"],
      [tagsOk, "One to three recruiting tags"],
      [fundsOk, "Filing fee of " + num(FEE) + " credits"]] as [boolean, string][])
      .forEach(([pass, label]) => {
        const hx = pass ? 0x5cff8a : 0xff4d5e;
        const g = new PIXI.Graphics();
        g.poly([16, y + 6, 22, y, 28, y + 6, 22, y + 12]).fill({ color: hx, alpha: pass ? 1 : 0.35 });
        sideL.addChild(g);
        const mk = lbl(pass ? "✓" : "✕", 6.5, pass ? 0x06140b : 0xffd0d5, 0);
        mk.anchor.set(0.5); mk.x = 22; mk.y = y + 6;
        sideL.addChild(mk);
        const l = val(label, 9, pass ? 0xdbe9fb : 0xff8c9b);
        l.x = 36; l.y = y;
        sideL.addChild(l);
        y += 20;
      });
    ...
    const create = makeButton({
      w: sw - 32, h: 34,
      label: ok ? "✦ FILE CHARTER · " + num(FEE) : "COMPLETE THE FORM",
      tone: "currency", enabled: ok,
      onClick: () => {
        if (!ok) {
          toast = {
            text: !nameOk ? "Fix the clan name first" : !tagOk ? "Fix the tag first"
              : !tagsOk ? "Pick at least one recruiting tag" : "Not enough credits for the filing fee",
            hex: 0xff8c9b,
          };
          build(); return;
        }
        o.onCreate?.({
          name: clean, tag, motto, shape: SHAPES[shape][0], symbol: SYMBOLS[symbol][0],
          crestColor: CREST_COLORS[crestC], glyphColor: GLYPH_COLORS[glyphC],
          admission, minLevel, focus: [...picks],
        });
        toast = { text: clean.toUpperCase() + " ⟨" + tag + "⟩ filed with the registry", hex: 0x5cff8a };
        build();
      },
    });
    create.x = 16; create.y = shell.bodyH - 92;
    const clear = makeButton({
      w: sw - 32, h: 30, label: "CLEAR FORM", tone: "steel",
      onClick: () => {
        name = ""; tag = ""; motto = ""; picks.length = 0;
        shape = 0; symbol = 0; crestC = 0; glyphC = 0;
        admission = "apply"; minLevel = 20; toast = null;
        fName.setValue(""); fTag.setValue(""); fMotto.setValue("");
        build();
      },
    });
    clear.x = 16; clear.y = shell.bodyH - 52;
    sideL.addChild(create, clear);

    if (toast) {
      const tv2 = val(toast.text, 9, toast.hex);
      tv2.anchor.x = 0.5; tv2.x = sw / 2; tv2.y = shell.bodyH - 16;
      sideL.addChild(tv2);
    }
  };
  ...
  return {
    ...
    update: (dt: number): void => {
      t += dt;
      shell.update(dt);
      fName.tick(t); fTag.tick(t); fMotto.tick(t);
      for (const g of glows) g.sp.alpha = g.base + Math.sin(t * 1.6) * 0.1;
      if (sheen) sheen.x = ((t / 6) % 1) * (crestSize + sheen.width) - sheen.width;
    },
