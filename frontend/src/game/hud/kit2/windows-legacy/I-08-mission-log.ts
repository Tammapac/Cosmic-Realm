// I-08-mission-log · MISSION LOG
//
// Der schmale Log, der im Spiel offen bleibt: aktive Aufträge minimal aufgelistet
// mit Typ-Raute, Level, Titel und Fortschrittsröhre. Klick auf eine Zeile ruft
// onOpen — dort hängt das Briefing (I-09). Der Log bleibt beim Wechsel stehen; nur
// das Briefing druckt sich neu.
//
...
//
//   import { mount } from "./I-08-mission-log";
...
/** id, type, title, level, zone, steps done, steps total, hex */
type Mission = [string, string, string, number, string, number, number, string];

const MISSIONS: Mission[] = [
  ["m1", "BOUNTY", "Corsair Ace", 50, "Ember Fields · RIM 4-2", 2, 3, "#ff4d5e"],
  ["m2", "EVENT", "Nova Tide", 50, "Null Span · RIM 4-4", 1, 4, "#ff5cf0"],
  ["m3", "HAUL", "Foundry Run", 32, "Foundry Belt · EIC 1-4", 3, 3, "#e8b94d"],
  ["m4", "SURVEY", "Cobalt Seams", 27, "Cobalt Verge · EIC 1-2", 1, 2, "#4ee2ff"],
  ["m5", "PATROL", "Kepler Watch", 19, "Kepler Station · EIC 1-1", 4, 6, "#5cff8a"],
  ["m6", "ESCORT", "Solace Convoy", 41, "Solace Anchorage · VRU 3-1", 0, 2, "#b866ff"],
  ["m7", "SALVAGE", "Erebus Wreck", 46, "Drift Market · MMO 2-3", 2, 5, "#ff8c4d"],
];

export type MissionLogOpts = WindowOpts & {
  missions?: Mission[];
  /** Klick auf eine Zeile — hier das Briefing öffnen. */
  onOpen?: (id: string) => void;
};

export function mount(o: MissionLogOpts = {}): WindowHandle {
  const list = o.missions ?? MISSIONS;
  const W = 336, H = 60 + list.length * 46 + 26;

  const shell = windowShell({
    w: W, h: H, accent: ACCENT.action, chamfer: 24, duration: 1050,
    title: "Missions", note: undefined,
  ...
  shell.body.addChild(listL);

  const build = (): void => {
    listL.removeChildren();
    glows.length = 0;
    const lw = shell.bodyW;

    const cnt = val(list.length + " ACTIVE", 9, 0xc9b2e8);
    cnt.anchor.x = 1; cnt.x = lw; cnt.y = -30;
    listL.addChild(cnt);

    list.forEach((m, i) => {
      const [id, type, title, lvl, zone, done, total, hx] = m;
      const c = hex(hx), on = sel === id, full = done >= total;
      const y = i * 46;
      const row = new PIXI.Container();

      const g = new PIXI.Graphics();
      cut(g, 0, 0, lw, 42, 9, on ? shade(c, -0.8) : (i % 2 ? 0x0e0b16 : 0x100d1a), 1, "tl-br");
      g.rect(0, 0, 2, 42).fill({ color: c, alpha: on ? 1 : 0.6 });
      if (on) {
        g.rect(2, 1, lw - 4, 1).fill({ color: shade(c, 0.7), alpha: 0.35 });
        g.rect(0, 40, lw, 2).fill({ color: c, alpha: 0.4 });
      }
      row.addChild(g);

      const d = new PIXI.Graphics();
      d.poly([12, 14, 16, 18, 12, 22, 8, 18]).fill(c);
      row.addChild(d);
      if (full) {
        const dg = glow(c, 24, 24, 0.7);
        dg.x = 0; dg.y = 6;
        row.addChild(dg);
        glows.push({ sp: dg, base: 0.55 });
      }

      const ty = lbl(type, 5.5, c, 1.8);
      ty.x = 24; ty.y = 6;
      row.addChild(ty);
      const lv = val("LV " + lvl, 8, 0x7f93aa);
      lv.x = 24 + ty.width + 8; lv.y = 5;
      row.addChild(lv);
      ...
      nm.x = 24; nm.y = 16;
      row.addChild(nm);
      const zn = val(zone, 8, 0x6b7f96);
      zn.x = 24; zn.y = 30;
      row.addChild(zn);

      // Fortschrittsröhre rechts
      const bw = 62, bh = 7, bx = lw - bw - 12;
      const tr = new PIXI.Graphics();
      tr.rect(bx, 16, bw, bh).fill(0x05080f);
      tr.rect(bx, 16, bw, 1).fill({ color: 0x000000, alpha: 0.85 });
      tr.rect(bx, 16 + bh - 1, bw, 1).fill({ color: c, alpha: 0.16 });
      row.addChild(tr);
      const p = done / total, fw = p * bw;
      if (fw > 0) {
        const fl = new PIXI.Sprite(gradTex([
          [0, rgba(shade(c, 0.6), 1)], [0.48, rgba(c, 1)], [1, rgba(shade(c, -0.5), 1)],
        ]));
        fl.x = bx; fl.y = 16; fl.width = fw; fl.height = bh;
        row.addChild(fl);
        const fg = glow(c, fw + 14, bh * 3, 0.4);
        fg.x = bx - 7; fg.y = 16 - bh;
        row.addChild(fg);
        glows.push({ sp: fg, base: 0.36 });
      }
      const st = val(done + " / " + total, 8, full ? shade(ACCENT.confirm, 0.4) : 0xbad2ec);
      st.anchor.x = 1; st.x = lw - 12; st.y = 27;
      row.addChild(st);
      ...
      row.on("pointerup", () => { row.x = x0 + 3; sel = id; build(); o.onOpen?.(id); });
      listL.addChild(row);
    });
  };
  ...
  return {
    ...
    update: (dt: number): void => {
      t += dt;
      shell.update(dt);
      for (const g of glows) g.sp.alpha = g.base + Math.sin(t * 1.8) * 0.14;
    },
