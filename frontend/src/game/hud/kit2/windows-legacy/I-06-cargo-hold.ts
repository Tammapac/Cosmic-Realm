// I-06-cargo-hold · CARGO HOLD
//
// Der Frachtraum: Rohstoffliste mit Menge, Stationspreis und Zeilenwert, links der
// Füllstand als Röhre mit Plasmafüllung und Warnfarbe ab 85 %. Sortierbar nach
// Menge, Wert und Name. Unten SELL ALL und der goldene TRADE DRONE — ohne Premium
// ausgegraut, Klick erklärt in einem Dialog warum.
//
...
//
//   import { mount } from "./I-06-cargo-hold";
...
/** key, name, glyph, units, unit price, volume per unit, hex */
type Ore = [string, string, string, number, number, number, string];

const ORES: Ore[] = [
  ["prometium", "Prometium", "◆", 12480, 14, 1, "#4ee2ff"],
  ["endurium", "Endurium", "◇", 8640, 22, 1, "#5cff8a"],
  ["terbium", "Terbium", "◈", 5210, 38, 1, "#b866ff"],
  ["xenomit", "Xenomit", "✦", 1840, 126, 2, "#e8b94d"],
  ["prisma", "Prisma Shards", "❖", 620, 480, 3, "#ff5cf0"],
  ["seprom", "Seprom", "✧", 240, 1420, 4, "#ff8c4d"],
  ["voidsalt", "Void Salt", "◉", 96, 3800, 6, "#9df2ff"],
  ["slag", "Foundry Slag", "▪", 22400, 3, 1, "#8aa0c0"],
];

const CAPACITY = 96000;

export type CargoOpts = WindowOpts & {
  ores?: Ore[];
  capacity?: number;
  /** Ohne Premium bleibt die Handelsdrone gesperrt. */
  premium?: boolean;
  /** Innerhalb einer Station darf ohne Premium verkauft werden. */
  docked?: boolean;
};

export function mount(o: CargoOpts = {}): WindowHandle {
  const rows0 = o.ores ?? ORES;
  const cap = o.capacity ?? CAPACITY;
  ...
  const shell = windowShell({
    ...
    title: "Cargo Hold", note: "ore manifest · fill gauge · station valuation",
    onClosed: o.onClosed, autoplay: o.autoplay,
  });

  let sort: "units" | "value" | "name" = "value";
  ...
  listL.x = 236;
  shell.body.addChild(leftL, listL, overL);

  const glows: { sp: PIXI.Sprite; base: number }[] = [];
  let fillBar: { root: PIXI.Container; set: (p: number) => void } | null = null;
  let dialog: PIXI.Container | null = null;
  let fils: { sp: PIXI.Sprite; speed: number; phase: number }[] = [];

  const volOf = (r: Ore): number => r[3] * r[5];
  const valOf = (r: Ore): number => r[3] * r[4];
  const usedVol = (): number => rows0.reduce((a, r) => a + volOf(r), 0);
  const totalVal = (): number => rows0.reduce((a, r) => a + valOf(r), 0);

  const build = (): void => {
    leftL.removeChildren(); listL.removeChildren();
    glows.length = 0; fils = [];

    const used = usedVol(), pct = Math.min(1, used / cap);
    const warn = pct >= 0.85;
    const gaugeC = warn ? 0xff4d5e : ACCENT.system;

    /* Füllstand links */
    const gw = 220;
    const card = new PIXI.Graphics();
    cut(card, 0, 0, gw, 196, 12, 0x081016, 1, "tr-bl");
    card.rect(12, 0, gw - 24, 1).fill({ color: shade(gaugeC, 0.8), alpha: 0.35 });
    card.rect(8, 194, gw - 16, 2).fill({ color: gaugeC, alpha: 0.4 });
    leftL.addChild(card);
    const wash = new PIXI.Sprite(radTex([[0, rgba(gaugeC, 0.16)], [0.74, "rgba(0,0,0,0)"]]));
    wash.width = gw; wash.height = 120;
    leftL.addChild(wash);

    const gl = lbl("HOLD CAPACITY", 6.5, shade(gaugeC, 0.5), 2.6);
    gl.x = 14; gl.y = 14;
    leftL.addChild(gl);
    const pv = val(Math.round(pct * 100) + " %", 30, warn ? 0xffd0d5 : 0xcfefff);
    pv.x = 14; pv.y = 28;
    leftL.addChild(pv);
    const pu = val(num(used) + " / " + num(cap) + " UNITS", 9.5, 0xbad2ec);
    pu.x = 14; pu.y = 66;
    leftL.addChild(pu);

    // Röhre mit Plasmafüllung und Filamenten
    const bw = gw - 28, bh = 16;
    const track = new PIXI.Graphics();
    track.rect(14, 88, bw, bh).fill(0x05080f);
    track.rect(14, 88, bw, 1).fill({ color: 0x000000, alpha: 0.85 });
    track.rect(14, 88 + bh - 1, bw, 1).fill({ color: gaugeC, alpha: 0.16 });
    leftL.addChild(track);
    const holder = new PIXI.Container();
    const bclip = new PIXI.Graphics();
    bclip.rect(14, 88, bw, bh).fill(0xffffff);
    holder.addChild(bclip); holder.mask = bclip;
    leftL.addChild(holder);
    const fw = pct * bw;
    if (fw > 0) {
      const fill = new PIXI.Sprite(gradTex([
        [0, rgba(shade(gaugeC, 0.6), 1)], [0.48, rgba(gaugeC, 1)], [1, rgba(shade(gaugeC, -0.5), 1)],
      ]));
      fill.x = 14; fill.y = 88; fill.width = fw; fill.height = bh;
      holder.addChild(fill);
      for (let i = 0; i < 3; i++) {
        const sp = new PIXI.Sprite(gradTex([
          [0, "rgba(0,0,0,0)"], [0.5, rgba(shade(gaugeC, 0.85), 0.5)], [1, "rgba(0,0,0,0)"],
        ], false, 64));
        sp.width = bw * 0.2; sp.height = bh * 0.5;
        sp.y = 88 + bh * (0.18 + i * 0.24);
        sp.blendMode = "add";
        holder.addChild(sp);
        fils.push({ sp, speed: 0.13 + i * 0.05, phase: i * 0.37 });
      }
      const cap2 = new PIXI.Graphics();
      cap2.rect(14 + fw - 2, 87, 2, bh + 2).fill(shade(gaugeC, 0.8));
      holder.addChild(cap2);
      const bg = glow(gaugeC, fw + 24, bh * 3, 0.45);
      bg.x = 2; bg.y = 88 - bh;
      leftL.addChild(bg);
      glows.push({ sp: bg, base: 0.42 });
    }
    const ticks = new PIXI.Graphics();
    for (let x = 0; x < bw; x += 5) ticks.rect(14 + x, 88, 1, bh).fill({ color: 0x000000, alpha: 0.4 });
    ticks.eventMode = "none";
    leftL.addChild(ticks);
    fillBar = { root: holder, set: () => {} };

    if (warn) {
      const w2 = lbl("HOLD NEARLY FULL", 6.5, 0xff8c9b, 2.4);
      w2.x = 14; w2.y = 112;
      leftL.addChild(w2);
    }

    // Wertkennzahlen
    ([["MANIFEST VALUE", num(totalVal()) + " cr", "#e8b94d"],
      ["ORE TYPES", String(rows0.length), "#4ee2ff"],
      ["FREE VOLUME", num(Math.max(0, cap - used)), "#5cff8a"]] as [string, string, string][])
      .forEach(([k, v, hx], i) => {
        const c = hex(hx), y = 128 + i * 22;
        const g = new PIXI.Graphics();
        g.rect(14, y, gw - 28, 20).fill(0x060a10);
        g.rect(14, y, gw - 28, 1).fill({ color: 0x000000, alpha: 0.8 });
        g.rect(14, y + 19, gw - 28, 1).fill({ color: c, alpha: 0.14 });
        g.poly([22, y + 7.5, 25.5, y + 4, 29, y + 7.5, 25.5, y + 11]).fill(c);
        leftL.addChild(g);
        const kk = val(k, 8.5, 0xbad2ec); kk.x = 36; kk.y = y + 5;
        const vv = val(v, 9, 0xdbe9fb); vv.anchor.x = 1; vv.x = gw - 22; vv.y = y + 5;
        leftL.addChild(kk, vv);
      });

    /* Aktionen */
    const sellAll = makeButton({
      w: gw, h: 34, label: docked ? "SELL ALL · " + num(totalVal()) : "SELL ALL · DOCK FIRST",
      tone: "confirm", enabled: docked,
      onClick: () => {
        if (!docked) { gate("Selling needs a station dock — or a trade drone, which is a premium fitting."); return; }
        rows0.length = 0; build();
      },
    });
    sellAll.y = 210;
    leftL.addChild(sellAll);

    const drone = makeButton({
      w: gw, h: 36, label: premium ? "SEND TRADE DRONE" : "TRADE DRONE · PREMIUM",
      tone: "currency", enabled: premium,
      onClick: () => {
        if (!premium) { gate("A trade drone flies your ore to the station and sells it while you keep mining. The fitting comes with premium."); return; }
        rows0.length = 0; build();
      },
    });
    drone.y = 252;
    leftL.addChild(drone);
    if (premium) {
      const dg = glow(ACCENT.currency, gw * 1.15, 60, 0.3);
      dg.x = -gw * 0.075; dg.y = 244;
      leftL.addChildAt(dg, 0);
      glows.push({ sp: dg, base: 0.26 });
    }

    /* Sortierung */
    const lw = shell.bodyW - 236;
    ([["value", "VALUE"], ["units", "UNITS"], ["name", "NAME"]] as const).forEach(([k, label], i) => {
      const tab = makeTab({
        w: 78, h: 24, label, accent: ACCENT.system, active: sort === k,
    ...
    /* Kopfzeile */
    ([["ORE", 12, false], ["UNITS", lw - 216, true], ["PRICE", lw - 130, true], ["VALUE", lw - 12, true]] as [string, number, boolean][])
      .forEach(([label, cx, right]) => {
        const l = lbl(label, 6, 0x9fb6d4, 2.6);
    ...
    /* Zeilen */
    const list = [...rows0].sort((a, b) =>
      sort === "name" ? a[1].localeCompare(b[1])
        : sort === "units" ? b[3] - a[3] : valOf(b) - valOf(a));

    list.forEach((r, i) => {
      const c = hex(r[6]), on = sel === r[0], y = 54 + i * 30;
      const row = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 0, 0, lw, 28, 8, on ? shade(c, -0.8) : (i % 2 ? 0x0b1018 : 0x0d131c), 1, "tl-br");
      g.rect(0, 0, 2, 28).fill({ color: c, alpha: on ? 1 : 0.6 });
      if (on) {
        g.rect(2, 1, lw - 4, 1).fill({ color: shade(c, 0.7), alpha: 0.35 });
        g.rect(0, 26, lw, 2).fill({ color: c, alpha: 0.4 });
      }
      row.addChild(g);
      ...
      for (const [ii, tone] of [[0, 0.35], [2, -0.42]] as [number, number][]) {
        sock.poly(hexa(12 + ii, 4 + ii, 20 - ii * 2, 20 - ii * 2)).fill(shade(c, tone));
      }
      row.addChild(sock);
      const gy = new PIXI.Text({ text: r[2], style: { fontFamily: "Orbitron, sans-serif", fontSize: 9, fill: shade(c, 0.55) } });
      gy.anchor.set(0.5); gy.x = 22; gy.y = 14;
      row.addChild(gy);
      ...
      nm.x = 42; nm.y = 8;
      row.addChild(nm);
      const vol = val(r[5] + " vol/u", 8, 0x7f93aa);
      vol.x = 42 + nm.width + 10; vol.y = 10;
      row.addChild(vol);

      const u = val(num(r[3]), 10, 0xdbe9fb);
      u.anchor.x = 1; u.x = lw - 216; u.y = 8;
      const p = val(num(r[4]) + " cr", 9.5, 0xbad2ec);
      p.anchor.x = 1; p.x = lw - 130; p.y = 9;
      const v = val(num(valOf(r)), 10.5, shade(ACCENT.currency, 0.5));
      v.anchor.x = 1; v.x = lw - 12; v.y = 8;
      row.addChild(u, p, v);

      const dv = new PIXI.Graphics();
      for (const x of [lw - 208, lw - 122]) {
        dv.rect(x, 4, 1, 20).fill({ color: 0x000000, alpha: 0.55 });
        dv.rect(x + 1, 4, 1, 20).fill({ color: shade(c, 0.6), alpha: 0.07 });
      }
      row.addChild(dv);
      ...
      row.on("pointerup", () => { row.x = x0 + 3; sel = on ? null : r[0]; build(); });
      listL.addChild(row);
    });

    if (!list.length) {
      const e = val("The hold is empty.", 10, 0x6b7f96);
      e.x = 14; e.y = 62;
      listL.addChild(e);
    }
  };
  ...
  return {
    ...
    update: (dt: number): void => {
      t += dt;
      shell.update(dt);
      for (const g of glows) g.sp.alpha = g.base + Math.sin(t * 1.7) * 0.09;
      for (const f of fils) {
        const span = 220;
        f.sp.x = 14 + (((t * f.speed + f.phase) % 1) * (span + f.sp.width)) - f.sp.width;
        f.sp.alpha = 0.45 + Math.sin(t * 2.2 + f.phase * 6) * 0.28;
      }
      void fillBar;
    },
