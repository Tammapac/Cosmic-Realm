// I-04-zone-map · ZONE MAP
//
// Die Minimap in groß. Links die Auswertespalte mit Zonenname, Reichweitenwahl
// (2 / 5 / 10 km), Kontaktfiltern und Legende; rechts das Scope mit Stationen,
// Asteroidengürtel, Portalen, Void-Portalen, Fabriken, Gegnern, Party und dir.
// 
// Klick ins Scope setzt einen lila Zielmarker: der Pfeil dreht in den Kurs, ein
// Triebwerksglühen pulst, und die ganze Zone scrollt an dir vorbei, während die
// Entfernungen live mitzählen.
//
...
//
//   import { mount } from "./I-04-zone-map";
...
const CONTACTS: Contact[] = [
  ["st1", "station", "Kepler Station", 166, 148, "DOCK OPEN",
    "Zone hub. Repairs, refit, market access and the contract board."],
  ["st2", "station", "Relay Outpost", 648, 512, "TRADE ONLY",
    "Small relay. Sells ammunition and buys ore above station rate."],
  ["p1", "portal", "Gate to 1-4", 742, 132, "LV 24+",
    "Lane portal into the Foundry Belt. Level gate holds at twenty-four."],
  ["p2", "portal", "Gate to 1-2", 92, 470, "LV 8+",
    "Back door to the Cobalt Verge. Quiet, and usually empty."],
  ["v1", "void", "Void Portal", 500, 84, "GROUP 4",
    "Dungeon entry. Four hulls minimum, no re-entry once it seals."],
  ["f1", "factory", "Refinery Delta", 300, 546, "ONLINE",
    "Ore refinery. Processes prometium and endurium on a shift clock."],
  ["f2", "factory", "Munitions Works", 700, 300, "ONLINE",
    "Turns salvage into ammunition. Pays in credits or rounds."],
  ["h1", "enemy", "Corsair Ace", 420, 220, "LV 50",
    "Modified interceptor. Fast, shielded, and it commits as soon as you do."],
  ["h2", "enemy", "Corsair Wing", 452, 258, "LV 46",
    "Four hulls flying tight. They break formation only when the lead dies."],
  ["h3", "enemy", "Hive Drone", 300, 466, "LV 21",
    "Drone off the belt swarm. No shield, heavy hull, and it never disengages."],
  ["h4", "enemy", "Hive Cluster", 262, 430, "LV 19",
    "Six drones on a patrol arc. Individually harmless, together a problem."],
  ["pt1", "party", "Vega_9", 380, 340, "LV 58",
    "Holding the north approach. Shields at eighty-four percent."],
  ["pt2", "party", "Sable", 344, 372, "LV 55",
    "Pulled back to repair. Two minutes out."],
];

const BELT = { x: 260, y: 420, w: 420, h: 150, rot: -13 };
const RANGE: Record<number, number> = { 2: 2.5, 5: 1, 10: 0.5 };

export type ZoneMapOpts = WindowOpts & {
  zone?: string;
  zoneId?: string;
  contacts?: Contact[];
...
export function mount(o: ZoneMapOpts = {}): WindowHandle {
  const list = o.contacts ?? CONTACTS;
  const W = 1080, H = 700;
  const shell = windowShell({
    w: W, h: H, accent: ACCENT.system,
    title: "Zone Map", note: "sites · belts · portals · contacts",
    onClosed: o.onClosed, autoplay: o.autoplay,
  });

  const ZONE = o.zone ?? "Alpha Sector";
  const ZID = o.zoneId ?? "EIC 1-3";
  let rng = 5;
  let filter: string | null = null;
  let sel: string | null = "st1";
  const ship = { x: 420, y: 320 };
  const dest = { x: 0, y: 0, on: false };
  let flying = false;
  let head2 = 0;
  let t = 0;
  let raf = 0;
  ...
  const scopeL = new PIXI.Container();
  scopeL.x = 300;
  shell.body.addChild(colL, scopeL);

  const pulses: { sp: PIXI.Sprite; base: number }[] = [];
  let arrow: PIXI.Graphics | null = null;
  let thrust: PIXI.Sprite | null = null;
  let destNode: PIXI.Container | null = null;
  let sweepSprite: PIXI.Sprite | null = null;
  let scopeH = 0;

  const dist = (c: Contact): number => Math.hypot(c[3] - ship.x, c[4] - ship.y) * 0.0125;
  const brg = (c: Contact): string =>
    String(Math.round((Math.atan2(c[3] - ship.x, -(c[4] - ship.y)) * 180 / Math.PI + 360) % 360)).padStart(3, "0");

  const build = (): void => {
    colL.removeChildren(); scopeL.removeChildren();
    pulses.length = 0; arrow = null; thrust = null; destNode = null; sweepSprite = null;

    const cw = 288;
    const sw = shell.bodyW - 300;
    const sh = shell.bodyH;
    scopeH = sh;

    /* Auswertespalte */
    const card = new PIXI.Graphics();
    cut(card, 0, 0, cw, sh, 14, 0x081016, 1, "tr-bl");
    card.rect(14, 0, cw - 28, 1).fill({ color: 0x9df2ff, alpha: 0.3 });
    card.rect(10, sh - 2, cw - 20, 2).fill({ color: ACCENT.system, alpha: 0.4 });
    colL.addChild(card);
    const wash = new PIXI.Sprite(radTex([[0, rgba(ACCENT.system, 0.14)], [0.74, "rgba(0,0,0,0)"]]));
    wash.width = cw; wash.height = 120;
    colL.addChild(wash);

    const zl = lbl("CURRENT ZONE", 6, 0x7fb8d4, 2.6);
    zl.x = 16; zl.y = 14;
    colL.addChild(zl);
    const zn = new PIXI.Text({
      text: ZONE,
      style: { fontFamily: "Inter, system-ui, sans-serif", fontSize: 20, fontWeight: "700", fill: 0xf2f7ff },
    });
    zn.x = 16; zn.y = 26;
    colL.addChild(zn);
    const zid = val(ZID + "  ·  LV 16 – 26", 9.5, 0x9df2ff);
    zid.x = 16; zid.y = 52;
    colL.addChild(zid);

    /* Reichweite */
    const rl = lbl("SCOPE RANGE", 6, 0x7fb8d4, 2.6);
    rl.x = 16; rl.y = 76;
    colL.addChild(rl);
    [2, 5, 10].forEach((r, i) => {
      const on = rng === r;
      const b = makeTab({
        w: 82, h: 24, label: r + " KM", accent: ACCENT.system, active: on,
        onClick: () => { rng = r; build(); },
      });
      b.x = 16 + i * 86; b.y = 90;
      colL.addChild(b);
    ...
    /* Filter */
    const fl = lbl("CONTACTS", 6, 0x7fb8d4, 2.6);
    fl.x = 16; fl.y = 126;
    colL.addChild(fl);
    const kinds: [string, string][] = [["all", "ALL"], ...Object.entries(KIND).map(([k, v]) => [k, v[0]] as [string, string])];
    kinds.forEach(([k, label], i) => {
      const c = k === "all" ? ACCENT.system : hex(KIND[k][1]);
      const on = k === "all" ? !filter : filter === k;
      const bw = (cw - 40) / 2;
      const b = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 0, 0, bw, 22, 6, on ? shade(c, -0.2) : 0x0c1118, 1, "tl-br");
      cut(g, 1, 1, bw - 2, 20, 5.5, on ? shade(c, -0.6) : 0x070b11, 1, "tl-br");
      g.rect(0, 0, 2, 22).fill({ color: c, alpha: on ? 1 : 0.4 });
      g.rect(5, 1, bw - 10, 1).fill({ color: shade(c, 0.7), alpha: on ? 0.5 : 0.14 });
      b.addChild(g);
      const l = lbl(label, 5.5, on ? 0xf4f9ff : shade(c, 0.3), 1.6);
      l.x = 8; l.y = 8;
      b.addChild(l);
      const n = list.filter((x) => k === "all" || x[1] === k).length;
      const nv = val(String(n), 8, on ? 0xdbe9fb : 0x6b7f96);
      nv.anchor.x = 1; nv.x = bw - 8; nv.y = 7;
      b.addChild(nv);
      b.x = 16 + (i % 2) * (bw + 8);
      b.y = 140 + Math.floor(i / 2) * 26;
      states(b, {
        lift: 2, sink: 1,
        onClick: () => { filter = k === "all" || filter === k ? null : k; build(); },
      });
      colL.addChild(b);
    });

    /* Kontaktliste */
    const listY = 140 + Math.ceil(kinds.length / 2) * 26 + 12;
    const shown = list.filter((c) => !filter || c[1] === filter)
      .sort((a, b) => dist(a) - dist(b));
    const divd = new PIXI.Graphics();
    divd.rect(16, listY - 6, cw - 32, 1).fill({ color: 0x000000, alpha: 0.6 });
    divd.rect(16, listY - 5, cw - 32, 1).fill({ color: ACCENT.system, alpha: 0.14 });
    colL.addChild(divd);

    const rowsFit = Math.floor((sh - listY - 130) / 28);
    shown.slice(0, rowsFit).forEach((c, i) => {
      const [id, kind, name, , , tag] = c;
      const kc = hex(KIND[kind][1]), on = sel === id;
      const y = listY + i * 28;
      const row = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 16, 0, cw - 32, 26, 7, on ? shade(kc, -0.78) : (i % 2 ? 0x0a0f16 : 0x0c121a), 1, "tl-br");
      g.rect(16, 0, 2, 26).fill({ color: kc, alpha: on ? 1 : 0.6 });
      if (on) g.rect(16, 24, cw - 32, 2).fill({ color: kc, alpha: 0.4 });
      row.addChild(g);
      const gy = new PIXI.Text({ text: KIND[kind][2], style: { fontFamily: "Orbitron, sans-serif", fontSize: 9, fill: kc } });
      gy.x = 24; gy.y = 8;
      row.addChild(gy);
      const nv2 = new PIXI.Text({
        text: name,
        style: { fontFamily: "Inter, system-ui, sans-serif", fontSize: 10, fontWeight: "700", fill: on ? 0xffffff : 0xdeeafa },
      });
      nv2.x = 40; nv2.y = 3;
      row.addChild(nv2);
      const tv = val(tag, 7.5, shade(kc, 0.35));
      tv.x = 40; tv.y = 15;
      row.addChild(tv);
      const dv = val(dist(c).toFixed(1) + " km", 9, 0xbad2ec);
      dv.anchor.x = 1; dv.x = cw - 24; dv.y = 3;
      row.addChild(dv);
      const bv = val(brg(c) + "°", 7.5, 0x6b7f96);
      bv.anchor.x = 1; bv.x = cw - 24; bv.y = 15;
      row.addChild(bv);
      row.y = y;
      const x0 = row.x;
      ...
      row.on("pointerup", () => { row.x = x0 + 3; sel = on ? null : id; build(); });
      colL.addChild(row);
    });

    /* Akte des Kontakts */
    const c2 = list.find((x) => x[0] === sel);
    if (c2) {
      const kc = hex(KIND[c2[1]][1]);
      const dy = sh - 120;
      const g = new PIXI.Graphics();
      cut(g, 16, dy, cw - 32, 104, 10, shade(kc, -0.82), 1, "tr-bl");
      g.rect(28, dy, cw - 56, 1).fill({ color: shade(kc, 0.7), alpha: 0.3 });
      g.rect(24, dy + 102, cw - 48, 2).fill({ color: kc, alpha: 0.35 });
      colL.addChild(g);
      const kl = lbl(KIND[c2[1]][0], 6, kc, 2.2);
      kl.x = 28; kl.y = dy + 10;
      colL.addChild(kl);
      const nv3 = new PIXI.Text({
        text: c2[2],
        style: { fontFamily: "Inter, system-ui, sans-serif", fontSize: 12.5, fontWeight: "700", fill: 0xf2f7ff },
      });
      nv3.x = 28; nv3.y = 22;
      colL.addChild(nv3);
      const bd = txt(c2[6], 10, 0xc4daee, cw - 56);
      bd.x = 28; bd.y = 40;
      colL.addChild(bd);
    }
    ...
    cut(well, 0, 0, sw, sh, 14, 0x04070e, 1, "tr-bl");
    well.rect(12, 0, sw - 24, 1).fill({ color: 0x000000, alpha: 0.85 });
    well.rect(12, sh - 2, sw - 24, 2).fill({ color: ACCENT.system, alpha: 0.2 });
    scopeL.addChild(well);
    const clip = new PIXI.Graphics();
    cut(clip, 0, 0, sw, sh, 14, 0xffffff, 1, "tr-bl");
    const inner = new PIXI.Container();
    inner.addChild(clip); inner.mask = clip;
    scopeL.addChild(inner);

    const bgWash = new PIXI.Sprite(radTex([
      [0, "rgba(11,26,41,1)"], [0.62, "rgba(5,8,15,1)"], [1, "rgba(2,4,10,1)"],
    ]));
    bgWash.width = sw; bgWash.height = sh;
    inner.addChild(bgWash);

    const k = RANGE[rng];
    const CX = ship.x, CY = ship.y;
    const px = (x: number): number => sw / 2 + (x - CX) * k;
    const py = (y: number): number => sh / 2 + (y - CY) * k;

    // Entfernungsringe
    const rings = new PIXI.Graphics();
    for (let i = 1; i <= 4; i++) {
      const r = (i / 4) * (Math.min(sw, sh) / 2 - 12);
      rings.circle(sw / 2, sh / 2, r).stroke({ width: 1, color: 0x4ee2ff, alpha: 0.08 });
    }
    rings.moveTo(sw / 2, 0).lineTo(sw / 2, sh).stroke({ width: 1, color: 0x4ee2ff, alpha: 0.06 });
    rings.moveTo(0, sh / 2).lineTo(sw, sh / 2).stroke({ width: 1, color: 0x4ee2ff, alpha: 0.06 });
    rings.eventMode = "none";
    inner.addChild(rings);

    // Asteroidengürtel
    const belt = new PIXI.Container();
    const bg2 = new PIXI.Graphics();
    for (let i = 0; i < 90; i++) {
      const rx = ((i * 73.3) % BELT.w) - BELT.w / 2;
      const ry = ((i * 41.7) % BELT.h) - BELT.h / 2;
      const rr = ((i * 29) % 5) / 2 + 0.8;
      bg2.circle(rx, ry, rr).fill({ color: 0x8aa0c0, alpha: 0.12 + ((i * 13) % 10) / 40 });
    }
    belt.addChild(bg2);
    belt.x = px(BELT.x + BELT.w / 2); belt.y = py(BELT.y + BELT.h / 2);
    belt.rotation = BELT.rot * Math.PI / 180;
    belt.scale.set(k);
    inner.addChild(belt);
    const bl = lbl("ASTEROID BELT", 7, 0x8aa0c0, 3);
    bl.anchor.set(0.5);
    bl.x = px(BELT.x + BELT.w / 2); bl.y = py(BELT.y + BELT.h / 2);
    bl.rotation = BELT.rot * Math.PI / 180;
    bl.alpha = 0.4;
    inner.addChild(bl);

    // Kontakte
    for (const c of list) {
      const [id, kind, name, cx, cy, tag] = c;
      const dim = filter ? kind !== filter : false;
      const kc = hex(KIND[kind][1]);
      const on = sel === id;
      const size = kind === "station" ? 26 : kind === "void" ? 24 : kind === "portal" ? 22 : 16;
      const node = new PIXI.Container();
      node.x = px(cx); node.y = py(cy);
      node.alpha = dim ? 0.3 : 1;

      const gl = glow(kc, size * 3, size * 3, on ? 0.6 : 0.32);
      gl.x = -size * 1.5; gl.y = -size * 1.5;
      node.addChild(gl);
      pulses.push({ sp: gl, base: on ? 0.5 : 0.28 });

      const g = new PIXI.Graphics();
      if (kind === "enemy") {
        g.poly([0, -size / 2, size / 2, size / 2, -size / 2, size / 2]).fill(shade(kc, 0.2));
        g.poly([0, -size / 2 + 3, size / 2 - 3, size / 2 - 2, -size / 2 + 3, size / 2 - 2]).fill(shade(kc, -0.5));
      } else if (kind === "factory") {
        g.rect(-size / 2, -size / 2, size, size).fill(shade(kc, 0.2));
        g.rect(-size / 2 + 2.5, -size / 2 + 2.5, size - 5, size - 5).fill(shade(kc, -0.5));
      } else if (kind === "party") {
        g.poly([0, -size / 2, size / 2, 0, 0, size / 2, -size / 2, 0]).fill(shade(kc, 0.3));
        g.poly([0, -size / 2 + 3, size / 2 - 3, 0, 0, size / 2 - 3, -size / 2 + 3, 0]).fill(shade(kc, -0.45));
      } else {
        for (const [inset, tone] of [[0, 0.4], [2.5, -0.1], [5, -0.55]] as [number, number][]) {
          g.poly(hexa(inset - size / 2, inset - size / 2, size - inset * 2, size - inset * 2)).fill(shade(kc, tone));
        }
      }
      node.addChild(g);

      if (kind === "void" || kind === "portal") {
        const gy = new PIXI.Text({ text: KIND[kind][2], style: { fontFamily: "Orbitron, sans-serif", fontSize: size * 0.4, fill: shade(kc, 0.6) } });
        gy.anchor.set(0.5);
        node.addChild(gy);
      }

      const nl = lbl(name.toUpperCase(), 6, shade(kc, 0.35), 1.4);
      nl.anchor.set(0.5, 0); nl.y = size / 2 + 4;
      node.addChild(nl);
      const tl = val(tag, 7, 0x7f93aa);
      tl.anchor.set(0.5, 0); tl.y = size / 2 + 14;
      node.addChild(tl);

      node.eventMode = "static";
      node.cursor = "pointer";
      node.on("pointerup", (e: PIXI.FederatedPointerEvent) => { e.stopPropagation(); sel = id; build(); });
      inner.addChild(node);
    }

    // Zielmarker
    destNode = new PIXI.Container();
    destNode.visible = dest.on && flying;
    const dg = new PIXI.Graphics();
    dg.poly([0, -9, 9, 0, 0, 9, -9, 0]).fill(shade(ACCENT.action, 0.4));
    destNode.addChild(dg);
    const dgl = glow(ACCENT.action, 44, 44, 0.6);
    dgl.x = dgl.y = -22;
    destNode.addChildAt(dgl, 0);
    const dl = val(Math.hypot(dest.x - ship.x, dest.y - ship.y).toFixed(0) + " u", 7, 0xeddcff);
    dl.anchor.set(0.5, 0); dl.y = 12;
    destNode.addChild(dl);
    destNode.x = px(dest.x); destNode.y = py(dest.y);
    inner.addChild(destNode);

    // eigenes Schiff
    thrust = new PIXI.Sprite(radTex([[0, "rgba(78,226,255,.3)"], [0.7, "rgba(0,0,0,0)"]]));
    thrust.width = thrust.height = 54;
    thrust.x = sw / 2 - 27; thrust.y = sh / 2 - 27;
    thrust.blendMode = "add";
    thrust.visible = flying;
    inner.addChild(thrust);

    arrow = new PIXI.Graphics();
    arrow.poly([0, -11, 7, 7, 0, 3, -7, 7]).fill(0x9fe0ff);
    arrow.x = sw / 2; arrow.y = sh / 2;
    arrow.rotation = head2;
    inner.addChild(arrow);
    const ag = glow(0x4ee2ff, 40, 40, 0.6);
    ag.x = sw / 2 - 20; ag.y = sh / 2 - 20;
    inner.addChildAt(ag, inner.children.length - 1);
    pulses.push({ sp: ag, base: 0.5 });
    const you = lbl("YOU", 6, 0x9fe0ff, 2.4);
    you.anchor.set(0.5, 0);
    you.x = sw / 2; you.y = sh / 2 + 14;
    inner.addChild(you);

    // Klick fliegt
    bgWash.eventMode = "static";
    bgWash.cursor = "crosshair";
    bgWash.on("pointerup", (e: PIXI.FederatedPointerEvent) => {
      const lp = e.getLocalPosition(inner);
      const wx = Math.max(40, Math.min(800, ship.x + (lp.x - sw / 2) / k));
      const wy = Math.max(30, Math.min(620, ship.y + (lp.y - sh / 2) / k));
      fly(wx, wy);
    });
  };

  const fly = (wx: number, wy: number): void => {
    const sx = ship.x, sy = ship.y;
    const d = Math.hypot(wx - sx, wy - sy);
    if (d < 4) return;
    cancelAnimationFrame(raf);
    dest.x = wx; dest.y = wy; dest.on = true;
    flying = true;
    head2 = Math.atan2(wx - sx, -(wy - sy));
    o.onFly?.(wx, wy);
    const dur = Math.min(2400, 440 + d * 4.6), t0 = performance.now();
    const step = (now: number): void => {
      ...
      else { flying = false; dest.on = false; build(); }
    };
  ...
  return {
    ...
    update: (dt: number): void => {
      t += dt;
      shell.update(dt);
      for (const p of pulses) p.sp.alpha = p.base + Math.sin(t * 1.9) * 0.12;
      if (thrust && flying) thrust.alpha = 0.6 + Math.sin(t * 7) * 0.3;
      void sweepSprite; void scopeH;
    },
    destroy: (): void => { cancelAnimationFrame(raf); shell.root.destroy({ children: true }); },
  };
}
