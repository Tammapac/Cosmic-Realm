// I-12-pilot-dossier · PILOT DOSSIER
//
// Das Spielerprofil. Oben links Rang, Honor mit Fortschritt zum nächsten Rang,
// Name und Firma. Darunter die Attributverteilung: Damage 1 %, Hitpoints 1,5 %,
// Max Shield 1,5 %, Speed 0,5 % je Punkt — Zuteilung in 1× / 5× / 10× Schritten
// über grüne Quadratknöpfe. Rechts die Karrierepfade, die durch Handel, Kills,
// Mining und Missionen steigen, und darunter die Abschussliste.
//
...
//
//   import { mount } from "./I-12-pilot-dossier";
...
/** key, label, per point, unit, hex, glyph */
const ATTRS: [string, string, number, string, string, string][] = [
  ["dmg", "Damage", 1.0, "%", "#ff4d5e", "◆"],
  ["hp", "Hitpoints", 1.5, "%", "#5cff8a", "▲"],
  ["shield", "Max Shield", 1.5, "%", "#4ee2ff", "◈"],
  ["speed", "Speed", 0.5, "%", "#e8b94d", "✦"],
];

/** key, label, level, xp, xpMax, source, hex */
const PATHS: [string, string, number, number, number, string, string][] = [
  ["trade", "Merchant", 24, 68400, 92000, "Credits earned through trade and hauling", "#e8b94d"],
  ["combat", "Warrior", 31, 41200, 68000, "Player and NPC kills", "#ff4d5e"],
  ["mining", "Prospector", 19, 22800, 42000, "Ore refined and volume hauled", "#4ee2ff"],
  ["contracts", "Operative", 27, 55100, 74000, "Contracts cleared", "#b866ff"],
  ["explore", "Pathfinder", 14, 9600, 26000, "Zones charted and portals cleared", "#5cff8a"],
];
...
export type DossierOpts = WindowOpts & {
  name?: string;
  faction?: string;
  rank?: number;
  rankName?: string;
...
export function mount(o: DossierOpts = {}): WindowHandle {
  const W = 1000, H = 620;
  const shell = windowShell({
    w: W, h: H, accent: ACCENT.system,
    title: "Pilot Dossier", note: "rank · attributes · career paths · service record",
    onClosed: o.onClosed, autoplay: o.autoplay,
  });

  const NAME = o.name ?? "Nyx_7";
  const FAC = o.faction ?? "EIC";
  const rank = o.rank ?? 11;
  const rankName = o.rankName ?? "COMMANDER";
  const honor = o.honor ?? 96400;
  const honorNext = o.honorNext ?? 120000;
  let spare = o.spare ?? 18;
  const spent: Record<string, number> = Object.assign({ dmg: 24, hp: 18, shield: 16, speed: 9 }, o.spent ?? {});
  let stepIdx = 0;
  const STEPS = [1, 5, 10];
  let t = 0;
  ...
  const build = (): void => {
    ...
    /* Rang, Honor, Name, Firma */
    const idCard = new PIXI.Graphics();
    cut(idCard, 0, 0, lw, 128, 12, 0x081016, 1, "tr-bl");
    idCard.rect(12, 0, lw - 24, 1).fill({ color: 0x9df2ff, alpha: 0.32 });
    idCard.rect(8, 126, lw - 16, 2).fill({ color: ACCENT.system, alpha: 0.4 });
    leftL.addChild(idCard);
    const idWash = new PIXI.Sprite(radTex([[0, rgba(ACCENT.system, 0.16)], [0.74, "rgba(0,0,0,0)"]]));
    idWash.width = lw; idWash.height = 100;
    leftL.addChild(idWash);

    // Rangabzeichen im Achteck
    const bs = 72;
    const badge = new PIXI.Graphics();
    const oct = (inset: number): number[] => {
      const s = bs - inset * 2, x = 16 + inset, y = 16 + inset, q = s * 0.29;
      return [x + q, y, x + s - q, y, x + s, y + q, x + s, y + s - q,
        x + s - q, y + s, x + q, y + s, x, y + s - q, x, y + q];
    };
    for (const [inset, tone] of [[0, 0.45], [2.5, -0.1], [5, -0.52], [7.5, -0.78]] as [number, number][]) {
      badge.poly(oct(inset)).fill(shade(ACCENT.system, tone));
    }
    leftL.addChild(badge);
    const rn = val(String(rank), 26, 0xcfefff);
    rn.anchor.set(0.5); rn.x = 16 + bs / 2; rn.y = 16 + bs / 2 - 4;
    leftL.addChild(rn);
    const rl = lbl("RANK", 5.5, 0x9df2ff, 2);
    rl.anchor.set(0.5); rl.x = 16 + bs / 2; rl.y = 16 + bs - 18;
    leftL.addChild(rl);
    const bg = glow(ACCENT.system, bs * 1.7, bs * 1.7, 0.35);
    bg.x = 16 - bs * 0.35; bg.y = 16 - bs * 0.35;
    leftL.addChildAt(bg, 1);
    glows.push({ sp: bg, base: 0.3 });
    ...
    nm.x = 104; nm.y = 20;
    leftL.addChild(nm);
    const rname = lbl(rankName, 8, 0x9df2ff, 2.6);
    rname.x = 104; rname.y = 46;
    leftL.addChild(rname);
    const fac = lbl(FAC + " · EARTH INDUSTRIES CORP", 6.5, 0x7fb8d4, 2.2);
    fac.x = 104; fac.y = 60;
    leftL.addChild(fac);

    // Honor-Röhre
    const hp = Math.min(1, honor / honorNext);
    const hw = lw - 120, hh = 12;
    const ht = new PIXI.Graphics();
    ht.rect(104, 80, hw, hh).fill(0x05080f);
    ht.rect(104, 80, hw, 1).fill({ color: 0x000000, alpha: 0.85 });
    ht.rect(104, 80 + hh - 1, hw, 1).fill({ color: ACCENT.relic, alpha: 0.16 });
    leftL.addChild(ht);
    const hf = new PIXI.Sprite(gradTex([
      [0, rgba(shade(ACCENT.relic, 0.6), 1)], [0.48, rgba(ACCENT.relic, 1)], [1, rgba(shade(ACCENT.relic, -0.5), 1)],
    ]));
    hf.x = 104; hf.y = 80; hf.width = hp * hw; hf.height = hh;
    leftL.addChild(hf);
    const hg = glow(ACCENT.relic, hp * hw + 20, hh * 3, 0.42);
    hg.x = 94; hg.y = 80 - hh;
    leftL.addChild(hg);
    glows.push({ sp: hg, base: 0.4 });
    const hticks = new PIXI.Graphics();
    for (let x = 0; x < hw; x += 5) hticks.rect(104 + x, 80, 1, hh).fill({ color: 0x000000, alpha: 0.4 });
    leftL.addChild(hticks);
    const hl = lbl("HONOR", 6, 0xffc9f6, 2.4);
    hl.x = 104; hl.y = 98;
    leftL.addChild(hl);
    const hv = val(num(honor) + " / " + num(honorNext) + "  ·  " + num(honorNext - honor) + " TO RANK " + (rank + 1), 9, 0xdbe9fb);
    hv.anchor.x = 1; hv.x = 104 + hw; hv.y = 97;
    leftL.addChild(hv);

    /* Attribute */
    const aY = 142;
    const ah = lbl("ATTRIBUTE POINTS", 7, 0x9fb6d4, 3);
    ah.x = 0; ah.y = aY;
    leftL.addChild(ah);
    const sp2 = val(spare + " SPARE", 11, spare > 0 ? shade(ACCENT.confirm, 0.4) : 0x6b7f96);
    sp2.anchor.x = 1; sp2.x = lw; sp2.y = aY - 3;
    leftL.addChild(sp2);

    // Schrittweite
    STEPS.forEach((s, i) => {
      const on = stepIdx === i, bw = 42;
      const b = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 0, 0, bw, 22, 6, on ? shade(ACCENT.confirm, -0.2) : 0x101820, 1, "tl-br");
      cut(g, 1, 1, bw - 2, 20, 5.5, on ? shade(ACCENT.confirm, -0.6) : 0x080d14, 1, "tl-br");
      g.rect(4, 1, bw - 8, 1).fill({ color: shade(ACCENT.confirm, 0.7), alpha: on ? 0.6 : 0.18 });
      g.rect(3, 20, bw - 6, 2).fill({ color: ACCENT.confirm, alpha: on ? 1 : 0.22 });
      b.addChild(g);
      const l = lbl(s + "×", 7, on ? 0xd8ffe6 : 0x6b7f96, 1.2);
      l.anchor.set(0.5); l.x = bw / 2; l.y = 11;
      b.addChild(l);
      b.x = i * 48; b.y = aY + 16;
      states(b, { lift: 2, sink: 1, onClick: () => { stepIdx = i; build(); } });
      leftL.addChild(b);
    });

    const step = STEPS[stepIdx];
    ATTRS.forEach(([key, label, per, unit, hx, glyph], i) => {
      const c = hex(hx), pts = spent[key] ?? 0;
      const y = aY + 48 + i * 52;
      const row = new PIXI.Container();
      const g = new PIXI.Graphics();
      cut(g, 0, 0, lw, 46, 10, i % 2 ? 0x0b1018 : 0x0d131c, 1, "tl-br");
      g.rect(0, 0, 2, 46).fill({ color: c, alpha: 0.7 });
      g.rect(10, 1, lw - 20, 1).fill({ color: shade(c, 0.7), alpha: 0.16 });
      row.addChild(g);

      const sock = new PIXI.Graphics();
      for (const [ii, tone] of [[0, 0.4], [2.5, -0.34]] as [number, number][]) {
        sock.poly(hexa(12 + ii, 9 + ii, 28 - ii * 2, 28 - ii * 2)).fill(shade(c, tone));
      }
      row.addChild(sock);
      const gy = new PIXI.Text({ text: glyph, style: { fontFamily: "Orbitron, sans-serif", fontSize: 12, fill: shade(c, 0.55) } });
      gy.anchor.set(0.5); gy.x = 26; gy.y = 23;
      row.addChild(gy);

      const nml = new PIXI.Text({
        text: label,
        style: { fontFamily: "Inter, system-ui, sans-serif", fontSize: 12, fontWeight: "700", fill: 0xe6f3ff },
      });
      nml.x = 50; nml.y = 9;
      row.addChild(nml);
      const eff = val("+" + (per * pts).toFixed(1).replace(/\.0$/, "") + unit + "  ·  " + per + unit + " per point", 8.5, 0x9fb6d4);
      eff.x = 50; eff.y = 26;
      row.addChild(eff);

      const pv = val(String(pts), 15, c);
      pv.anchor.x = 1; pv.x = lw - 86; pv.y = 14;
      row.addChild(pv);

      // Quadratknöpfe: − rot, + grün
      const sq = (glyph2: string, accent: number, at: number, enabled: boolean, onClick: () => void): void => {
        const size = 26, b = new PIXI.Container();
        const gg = new PIXI.Graphics();
        gg.rect(0, 3, size, size).fill({ color: 0x03050a, alpha: 0.9 });
        gg.rect(0, 0, size, size).fill(shade(accent, 0.45));
        gg.rect(1.5, 1.5, size - 3, size - 3).fill(shade(accent, -0.36));
        gg.rect(3, 3, size - 6, size - 6).fill(shade(accent, -0.74));
        gg.rect(4, 3, size - 8, 1).fill({ color: shade(accent, 0.8), alpha: 0.7 });
        gg.rect(3, size - 5, size - 6, 2).fill({ color: accent, alpha: 0.75 });
        b.addChild(gg);
        const tg = new PIXI.Text({
          text: glyph2,
          style: { fontFamily: "Orbitron, sans-serif", fontSize: 14, fontWeight: "700", fill: shade(accent, 0.8) },
        });
        tg.anchor.set(0.5); tg.x = size / 2; tg.y = size / 2;
        b.addChild(tg);
        b.x = at; b.y = 10;
        states(b, { onClick, enabled });
        row.addChild(b);
      };
      sq("−", 0xff4d5e, lw - 72, pts > 0, () => {
        const d = Math.min(step, pts);
        spent[key] = pts - d; spare += d; build();
      });
      sq("+", 0x5cff8a, lw - 36, spare > 0, () => {
        const d = Math.min(step, spare);
        spent[key] = pts + d; spare -= d; build();
      });
    ...
    /* Karrierepfade */
    const rw = shell.bodyW - 468;
    const ph = lbl("CAREER PATHS", 7, 0x9fb6d4, 3);
    rightL.addChild(ph);

    PATHS.forEach(([, label, lvl, xp, xpMax, source, hx], i) => {
      const c = hex(hx), p = Math.min(1, xp / xpMax);
      const y = 18 + i * 52;
      const g = new PIXI.Graphics();
      cut(g, 0, y, rw, 46, 10, i % 2 ? 0x0b1018 : 0x0d131c, 1, "tl-br");
      g.rect(0, y, 2, 46).fill({ color: c, alpha: 0.7 });
      g.rect(10, y + 1, rw - 20, 1).fill({ color: shade(c, 0.7), alpha: 0.16 });
      rightL.addChild(g);

      const lv = val(String(lvl).padStart(2, "0"), 15, c);
      lv.x = 14; lv.y = y + 8;
      rightL.addChild(lv);
      const nm2 = new PIXI.Text({
        text: label,
        style: { fontFamily: "Inter, system-ui, sans-serif", fontSize: 12, fontWeight: "700", fill: 0xe6f3ff },
      });
      nm2.x = 48; nm2.y = y + 7;
      rightL.addChild(nm2);
      const src = val(source, 8, 0x7f93aa);
      src.x = 48; src.y = y + 24;
      rightL.addChild(src);
      ...
      tr.rect(bx, y + 12, bw2, bh2).fill(0x05080f);
      tr.rect(bx, y + 12, bw2, 1).fill({ color: 0x000000, alpha: 0.85 });
      tr.rect(bx, y + 12 + bh2 - 1, bw2, 1).fill({ color: c, alpha: 0.16 });
      rightL.addChild(tr);
      const fl = new PIXI.Sprite(gradTex([
        [0, rgba(shade(c, 0.6), 1)], [0.48, rgba(c, 1)], [1, rgba(shade(c, -0.5), 1)],
      ]));
      fl.x = bx; fl.y = y + 12; fl.width = p * bw2; fl.height = bh2;
      rightL.addChild(fl);
      const fg = glow(c, p * bw2 + 16, bh2 * 3, 0.4);
      fg.x = bx - 8; fg.y = y + 12 - bh2;
      rightL.addChild(fg);
      glows.push({ sp: fg, base: 0.36 });
      const xv = val(Math.round(p * 100) + " %", 8.5, 0xbad2ec);
      xv.anchor.x = 1; xv.x = rw - 12; xv.y = y + 25;
      rightL.addChild(xv);
    });

    /* Abschussliste */
    const kY = 18 + PATHS.length * 52 + 14;
    const kh = lbl("SERVICE RECORD", 7, 0x9fb6d4, 3);
    kh.y = kY;
    rightL.addChild(kh);
    const div = new PIXI.Graphics();
    div.rect(0, kY + 14, rw, 1).fill({ color: 0x000000, alpha: 0.6 });
    div.rect(0, kY + 15, rw, 1).fill({ color: ACCENT.system, alpha: 0.14 });
    rightL.addChild(div);

    KILLS.forEach(([k, v, hx], i) => {
      const c = hex(hx), y = kY + 22 + i * 24;
      const g = new PIXI.Graphics();
      g.rect(0, y, rw, 22).fill(i % 2 ? 0x080d14 : 0x0a1018);
      g.rect(0, y, rw, 1).fill({ color: 0x000000, alpha: 0.55 });
      g.rect(0, y + 21, rw, 1).fill({ color: c, alpha: 0.12 });
      g.poly([10, y + 8.5, 13.5, y + 5, 17, y + 8.5, 13.5, y + 12]).fill(c);
      rightL.addChild(g);
      const kk = val(k, 8.5, 0xbad2ec); kk.x = 26; kk.y = y + 6;
      const vv = val(v, 10, shade(c, 0.45)); vv.anchor.x = 1; vv.x = rw - 10; vv.y = y + 5;
      rightL.addChild(kk, vv);
    });
  };
