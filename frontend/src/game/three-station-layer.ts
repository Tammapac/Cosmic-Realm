...
export function getStationDoorWorldOffset(id: string): { x: number; y: number } | null {
  ...
  // world plane is (x, z). The offset is the HangarRamp node's world position minus
  // the station's — i.e. the REAL hangar-mouth position. Verified against the model:
  // the ramp/opening (tripo_part_4) sits ~43-51 world units out from the station
  // centre toward +Z (screen-bottom, where door stations are pinned), and in-game
  // the opening renders LARGER than the ship, so this small, true offset is correct.
  // (An earlier version stretched this to ~756 units, which flung the dock target
  // far past the station — removed.)
  return {
